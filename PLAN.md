...
- `LegacyCompatTests` now runs by default (the `SQLPROJECTSYNC_RUN_LEGACY` env-var gate was removed). The suite reports a real gap: **DacFx's `PublishChangesToProject` does not patch the legacy `.sqlproj` XML**. The on-disk `.sql` files are added/removed correctly, but the corresponding `<Build Include="..."/>` items are not added on file creation and not removed on file deletion. Two tests (`Apply_AddsSqlFile_AndBuildItem_…` and `Apply_RemovesSqlFile_AndBuildItem_…`) pin this behavior and fail; `Compare_NoFileLevelChanges_…` passes. The `legacy-compat.yml` nightly workflow stays on `continue-on-error: true` so these don't block CI.
- We confirmed via inspection of DacFx 170.x's public surface, the sqltoolsservice reference at `SchemaComparePublishProjectChangesOperation.cs`, and the GitHub repo that **no overload, parameter, or sibling API patches the legacy XML** — the limitation is a real DacFx omission, not a missed call site on our end.
- **The preview `Microsoft.SqlServer.DacFx.Projects` package (0.6.3-preview, namespace `Microsoft.SqlServer.Dac.Projects`) closes the gap.** Concretely: it's ~78 KB, ships a single DLL, and adds `Microsoft.Build` (compile-only via `PrivateAssets`) and `Microsoft.Extensions.FileSystemGlobbing` to the transitive set. The `SqlProject.OpenProject(path, readOnly: false)` API exposes `SqlProjStyle == LegacyStyle` for legacy projects and a `LegacyProjectScriptCollection`-typed `SqlObjectScripts` with `Add(new SqlObjectScript("dbo\\Tables\\Foo.sql"))` and `Delete("dbo\\Tables\\Foo.sql")` — verified against our `LegacyTestProject` fixture: both operations mutate the `<Build Include="…"/>` items in the on-disk XML as expected. Caveat: the package depends on DacFx **`170.4.63-preview`** (not our stable `170.3.93`), so adopting it means bumping our core to the same preview line; revisit when DacFx 170.4 ships stable.
- Concrete next step if/when we want to fix the gap: wire a small internal helper that, after `SchemaSync.Apply`, opens the project via `SqlProject.OpenProject` and replays `publish.AddedFiles` / `publish.DeletedFiles` onto `project.SqlObjectScripts`. Only do this when `SqlProjectStyle.Detect(path) == Legacy` — SDK projects need no patching.
