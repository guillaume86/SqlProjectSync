...
- Adopting `Microsoft.SqlServer.DacFx.Projects` 0.6.3-preview required bumping `Microsoft.SqlServer.DacFx` from stable `170.3.93` to preview `170.4.77-preview`. Both packages are pinned in `Directory.Packages.props`. When DacFx ships 170.4 stable, return both pins to stable lines.
