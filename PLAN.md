...
- `LegacyCompatTests` are gated on `SQLPROJECTSYNC_RUN_LEGACY=1`. With the gate open the suite runs against LocalDB and reports a real gap: **DacFx's `PublishChangesToProject` does not patch the legacy `.sqlproj` XML**. The on-disk `.sql` files are added/removed correctly, but the corresponding `<Build Include="..."/>` items are not added on file creation and not removed on file deletion. Two tests (`Apply_AddsSqlFile_AndBuildItem_…` and `Apply_RemovesSqlFile_AndBuildItem_…`) pin this behavior and currently fail — kept on `continue-on-error` in the nightly workflow. The `Compare_NoFileLevelChanges_…` and the file-level halves of the Add/Remove tests pass.
 CLAUDE.md already flags (c) as the preferred path if structural edits become necessary.
